...
/**
 * "Hình ảnh" field: hidden file input + "+ Image" trigger + thumbnail grid.
 * Each newly selected file is validated with `isValidKudosImage` before
 * being added; rejected files surface `imageInvalidError` instead of being
 * silently dropped. Blob preview URLs are revoked on remove AND on unmount
 * to avoid leaking memory during a long-lived session.
 */
