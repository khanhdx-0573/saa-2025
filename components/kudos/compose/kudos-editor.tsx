...
import { createMentionExtension } from "@/components/kudos/compose/mention-suggestion";
import { KudosEditorLinkDialog } from "@/components/kudos/compose/kudos-editor-link-dialog";
import { KudosEditorToolbar } from "@/components/kudos/compose/kudos-editor-toolbar";
...
type KudosEditorProps = {
  ...
  /** Prefills the doc for editing an existing kudos (Kudos Card edit pencil).
   *  Omit for the compose flow's blank editor — read once at mount, not a
   *  controlled value (Tiptap owns the doc after that). */
...
export const KudosEditor = forwardRef<KudosEditorHandle, KudosEditorProps>(function KudosEditor(
  ...
  const editor = useEditor({
    ...
    editorProps: { attributes: { class: "min-h-[120px] p-3 focus:outline-none" } },
  ...
  // `initialContent` only sets the doc at construction — this mirrors that
  // same length into the counter once the editor instance is ready, so
  // pre-filled edits show the right count from the start instead of "0".
  useEffect(() => {
    if (!editor) return;
    const { length } = measureEditorText(editor.state.doc, MAX_KUDOS_CONTENT_CHARS);
    setCharCount(length);
  }, [editor]);
  ...
  return (
    <div className="flex flex-1 flex-col gap-1">
      ...
      <div className="relative rounded-b-lg border border-details-border bg-details-text-secondary-1">
        {isEmpty && (
          <p className="pointer-events-none absolute left-3 top-3 font-montserrat text-sm font-bold text-details-text-secondary-2">
            {t("contentPlaceholder")}
          </p>
        )}
        <EditorContent
          editor={editor}
          className="font-montserrat text-sm text-details-text-primary-2 [&_a]:text-details-text-primary-2 [&_a]:underline [&_blockquote]:border-l-2 [&_blockquote]:border-details-border [&_blockquote]:pl-4 [&_ol]:list-decimal [&_ol]:pl-6 [&_ul]:list-disc [&_ul]:pl-6"
        />
      </div>
      <div className={`flex items-center ${charCount > 0 ? "justify-between" : "justify-center"}`}>
        <p className="font-montserrat text-sm font-bold text-details-text-primary-2">{t("contentHelper")}</p>
        {charCount > 0 && (
