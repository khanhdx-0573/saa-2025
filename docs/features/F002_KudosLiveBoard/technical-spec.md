...
- **FR-006** — Kudos Card field set: sender block, decorative arrow, recipient block, timestamp
  ...
  button, Copy Link button, "Xem chi tiết" link. `kudos.content` is Tiptap-authored HTML (per F001's
  `kudos-editor.tsx`), not plain text — the card renders it as sanitized HTML (allowlisted tags/attrs via
  `lib/kudos/sanitize-content-html.ts`), never escaped as literal markup.
...
- **FR-011** — Spotlight board (B.6/B.7): header "{total kudos count} KUDOS" (real DB count), an in-canvas
  search bar (placeholder "Tìm kiếm", max 100 chars, client-side substring filter of loaded nodes — distinct
  from the top-level Sunner search box, see FR-021), a pan/zoom toggle, and a canvas of scattered recipient
  names sized by kudos-received count; hover → tooltip (name + last-received timestamp); click node → the
...
- **FR-020** — All user-facing strings go through next-intl (en.json + vi.json); no hardcoded copy in JSX
  after integration.
- **FR-021** — Top-level Sunner search box (`components/kudos/sunner-search-bar.tsx`), placed next to the
  compose entry bar (A.1), separate from the Spotlight in-canvas search (FR-011): "Tìm kiếm profile Sunner"
  placeholder, debounced (~250ms) server-side lookup via `searchProfiles` (the same query `RecipientField`
  uses in the F001 compose modal), a dropdown of matching profiles, selecting one navigates to
  `/profile/[id]`. Spec-gap correction: present in the MoMorph design but missed during original F002
  authoring, which documented only the Spotlight board's search.
...
- **US010 — Anonymous card.** As a Sunner I want anonymous kudos shown without exposing the sender. AC:
  ...
  (FR-009, BR-10)
- **US011 — Find a Sunner's profile from the board.** As a Sunner I want to search for a colleague by name
  next to the compose bar so I can jump straight to their profile. AC: typing debounces a profile lookup;
  matches appear in a dropdown; selecting one navigates to `/profile/[id]`; distinct from the Spotlight
  board's own in-canvas search. (FR-021)
...
- **SC-12** — After integration, all board strings resolve through en/vi catalogs; no hardcoded JSX copy.
  (FR-020)
- **SC-13** — Selecting a match in the top-level Sunner search box navigates to that Sunner's `/profile/[id]`.
  (US011, FR-021)
...
- Post-implementation fix-bug pass: `components/kudos/sunner-search-bar.tsx` (FR-021, new top-level search
  box), `lib/kudos/sanitize-content-html.ts` (FR-006 correction, HTML content rendering).
