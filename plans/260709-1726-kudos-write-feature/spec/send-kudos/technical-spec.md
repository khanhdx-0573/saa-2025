...
- The system must keep the Submit action disabled until recipient, content, and hashtag-count (>=1) are all valid, and must enable it once all three are satisfied.
...
- Submit button state: enabled only when (recipient selected) AND (content non-empty) AND (hashtag count between 1 and 5); otherwise disabled.
...
- Submit is disabled while recipient is empty; selecting a recipient from the autocomplete dropdown populates the field and clears the error.
- Submit is disabled while content is empty; a "Không được để trống" error is shown on a submit attempt with empty content.
- Cancel discards all composed data and closes the modal immediately, no confirmation.
- With recipient, content, and >=1 hashtag valid, Submit is enabled; submitting shows a loading state and closes the modal on success.
- Attempting to submit with recipient, content, and hashtags all empty shows all three required-field errors simultaneously.

**Requirements fulfilled:** auth gate; recipient selection; content requirement; submit validation gating; cancel/discard.
...
- Submitting with 0 hashtags shows a "Không được để trống" error and blocks submit.
...
No feature-list.md exists yet (single-feature plan-local draft, not a SYSTEM decomposition). Codes: TBD (draft) — allocated at promote.
...
No source code written yet — see `## User Stories` for planned data model and endpoints.
