...
| Empty recipient on submit | Submit with no recipient selected | Red border + "Không được để trống"-style error on recipient field; submit blocked | high |
| Empty content on submit | Submit with content field blank | "Không được để trống" error shown; submit blocked | high |
| Zero hashtags on submit | Submit with 0 hashtags selected | "Không được để trống" error on hashtag field; submit blocked | high |
...
| All required fields empty on submit | Submit with recipient, content, and hashtags all empty | All three required-field errors shown simultaneously; no submission occurs | high |
