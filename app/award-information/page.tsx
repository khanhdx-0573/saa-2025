...
/**
 * Static "coming soon"-style mock page behind the header's "Award
 * Information" tab (FR-005). No live data, no backend — placeholder copy
 * only, per the business context. Auth-gating is inherited automatically
 * from the existing global middleware (`proxy.ts` → `lib/supabase/middleware.ts`),
 * so this page needs no gating code of its own (FR-009).
 */
export default function AwardInformationPage() {
  const t = useTranslations("AwardInformationPage");
    ...
    <div className="flex min-h-screen flex-col bg-details-background">
      ...
      <main className="flex flex-1 flex-col items-start gap-6 px-6 py-16 lg:px-36 lg:py-24">
        ...
        <ul className="flex max-w-[720px] flex-col gap-3 font-montserrat text-base leading-8 text-details-text-secondary-1">
          <li>{t("bulletCriteria")}</li>
          <li>{t("bulletPrizes")}</li>
          <li>{t("bulletCeremony")}</li>
        </ul>
      </main>
